...
- 访谈阶段每轮只问 1 个关键问题。
- 追问必须提供可选项和自定义入口，但要按运行环境选择题型表达：有结构化控件时，优先使用 `request_user_input`；没有结构化控件时，再退化成 Markdown 选项。
- 在 Plan mode 等提供 `request_user_input` 的环境里，把问题写成“先选最接近的一项”或“先选当前最主要的一项”。如果界面自动带“推荐”标记，必须在题干里明确说明“请忽略界面上的推荐标记，它只是 UI 限制；如果都不准确，直接填空”。
- 不要主动推荐某个事实答案。只有最终结论和下一步行动可以给推荐；访谈阶段如果 UI 自动显示推荐，也只能把它当作界面限制来中和，不能顺势诱导。
- 原本需要多选的事实收集题，不要因为 UI 限制退回成纯 Markdown。改成连续的单焦点问题，逐轮收集“最主要原因”“第二个阻力”“最在意的成本”等。
- 如果运行环境提供无推荐、多选、自定义输入的结构化控件，优先使用它来收集并列事实。
- 只有在没有结构化控件时，才使用 Markdown 退化格式；退化格式必须写清楚：`可多选：A/B/C/D；也可以直接填写自定义答案`。
...
好的第一问通常直击瓶颈；在 Plan mode 里优先改写成可点击单选加自定义输入：

```text
为了判断是否必要，先确认它要解决的真实问题。请先选最接近的一项；忽略界面上的“推荐”标记，它只是 UI 限制。如果都不准确，直接填空。
